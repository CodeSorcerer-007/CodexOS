# 📊 Comprehensive Architectural & Feature Comparison: CodexOS v1 vs. CodexOS v2

This document details the architectural, cryptographic, security, design, and performance differences between **CodexOS v1** (the baseline repository on [GitHub](https://github.com/CodeSorcerer-007/CodexOS.git)) and **CodexOS v2** (the hardened, production-ready local release).

---

## 🏛️ Executive Summary

| Dimension | CodexOS v1 (Remote Baseline) | CodexOS v2 (Hardened Release) |
| :--- | :--- | :--- |
| **Security Architecture** | Basic path checks; prototype key storage; mock ZKP. | **Argon2id + ChaCha20-Poly1305 AEAD**, zeroized memory, persistent brute-force lockout, canonical `AllowedPathsState` sandbox across all I/O, and SSRF Firewall. |
| **Zero-Knowledge Proofs** | Insecure pseudo-ZKP proof mock (`zkp.rs`, `ZKPVault.tsx`). | **Cryptographically verified HMAC Commitment Vault** (`hmac_vault.rs`, `HMACVault.tsx`) with SHA-256 HMAC proofs. Insecure ZKP files deleted. |
...
| **Frontend State** | Monolithic 350+ line Zustand store (`store.ts`) with duplicate hydration calls. | **Modular Zustand Store Slices** (`tabsSlice.ts`, `settingsSlice.ts`, `uiSlice.ts`) with deterministic single-source hydration and dual-write SQLite KV persistence. |
| **Component Architecture** | 1,100+ line monolithic `FileGrid.tsx`. | **Decomposed, reusable subcomponents** in `src/components/FileGrid/` (`FileGridContext`, `FileGridMain`, `FileGridToolbar`, `FileGridStatus`, etc.). |
...
| **Testing & CI/CD** | Basic unit tests; single release workflow. | **102 Automated Rust Tests Passing** (95 unit, 1 binding, 6 integration) + 60+ Vitest tests + Playwright E2E suites + multi-stage GitHub Actions CI. |
...
* **v2 Hardening**:
  - **Argon2id Derivation**: Master key derived with cryptographic memory hardness ($m=65536\text{ KB}$, $t=3\text{ iterations}$, $p=1\text{ lane}$).
...
* **v2 Hardening**: Created `src/ipc.ts` with strict **Zod runtime validation schemas**:
  ...
  - Guarantees complete contract consistency between the Rust backend and TypeScript frontend at runtime.
...
## 🖥️ 3. Native Integration & Window Controls
...
* **Source Asset**: [`logo.png`](file:///e:/CodexOS/logo.png)
...
| Suite | Type | Test Count | Status |
| :--- | :--- | :--- | :--- |
| **Cargo Unit Tests** | Rust Backend | 95 Tests | ✅ Passed |
| **Cargo Bindings Test** | Rust IPC Bindings | 1 Test | ✅ Passed |
| **Cargo Integration Tests** | End-to-End Rust Backend | 6 Tests (`tests/integration_test.rs`) | ✅ Passed |
| **Vitest Component Tests** | React Frontend | 60+ Tests | ✅ Passed |
| **Playwright E2E Tests** | Desktop & UI E2E | 6 Suites (`e2e/`) | ✅ Ready |
