...
export const ZKPVault = () => {
  ...
  const handleVerify = async () => {
    try {
      const valid = await invoke<boolean>('verify_zk_proof', { 
    ...
    <div className="flex h-full w-full bg-[#0a0f18] text-white">
      ...
      <div className="flex-1 p-6 flex flex-col gap-6 bg-black/40">
        ...
        <div>
          ...
          <label className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-2 block">Verification Key</label>
          <input 
            type="text"
            value={verifyKey}
