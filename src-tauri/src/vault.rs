...
pub fn decrypt_vault(path: String, password: String, out_path: String) -> Result<(), String> {
    ...
    let mut salt = [0u8; 16];
    file.read_exact(&mut salt).map_err(|_| "Failed reading salt".to_string())?;
    
    let mut nonce_bytes = [0u8; 12];
    file.read_exact(&mut nonce_bytes).map_err(|_| "Failed reading nonce".to_string())?;
    ...
    let plaintext = cipher.decrypt(nonce, ciphertext.as_ref()).map_err(|_| "Decryption failed! Incorrect password.".to_string())?;
    
    let mut archive = zip::ZipArchive::new(std::io::Cursor::new(plaintext)).map_err(|e| e.to_string())?;
