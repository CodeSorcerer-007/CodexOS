...
mod tests {
    ...
    fn test_cryptographic_commitment() {
        ...
        // 1. Generate Proof
        let proof = generate_zk_proof(vault.clone(), key.clone()).unwrap();
        assert!(proof.contains(':'));

        // 2. Verify with correct data
        let is_valid = verify_zk_proof(proof.clone(), vault.clone(), key.clone()).unwrap();
        ...
        let is_invalid = verify_zk_proof(proof.clone(), tampered_vault, key.clone()).unwrap();
        assert!(!is_invalid, "Tampered data should fail verification");
        
        // 4. Verify with wrong key
        let wrong_key = "MyPrivateKey124".to_string();
        let is_invalid_key = verify_zk_proof(proof, vault, wrong_key).unwrap();
