...
pub fn run_wasm_plugin(path: String) -> Result<String, String> {
    ...
    let instance = linker.instantiate(&mut store, &module).map_err(|e| format!("Failed to instantiate WASM: {}", e))?;
    
    let run_func = instance.get_typed_func::<(), ()>(&mut store, "run");
    
    match run_func {
        Ok(func) => {
            func.call(&mut store, ()).map_err(|e| format!("Failed to execute 'run': {}", e))?;
            Ok(format!("Successfully executed plugin: {}", path))
        },
        Err(_) => {
            Ok(format!("Successfully loaded plugin (no 'run' export found): {}", path))
        }
    }
}
